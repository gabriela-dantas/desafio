...
def select_new_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem novas cotas Porto md-cota, error:{error}")
        raise error
...
def select_updated_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem cotas atualizadas Porto md-cota, error:{error}")
        raise error
...
def select_new_auction_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem novas cotas leilão Porto md-cota, error:{error}")
        raise error
...
def select_new_no_auction_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem novas cotas não leilão Porto md-cota, error:{error}")
        raise error
...
def select_group_end_date_inconsistency(md_quota_cursor: psycopg2.cursor) -> dict:
    try:
        ...
        logger.error(f"Erro ao obter contagem de inconsistências para data de "
                     f"encerramento de grupos Porto md-cota, error:{error}")
        raise error
...
def select_auction_quotas_winner_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem de cotas ganhas no leilão Porto md-cota, error:{error}")
        raise error
...
def porto_kpi_calculates():
    ...
    try:
        ...
        logger.info(f"Quantidade de incosistências encontradas para data de encerramento de grupos Porto: "
        ...
        logger.info(f"Total de novas ofertas para Porto: {new_offers['new_offers']}")
