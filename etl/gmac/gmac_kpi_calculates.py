    ...
    try:
        query_select_new_quotas_md_quota = (
            ...
            FROM
                ...
                WHERE
                    pa.administrator_code = '0000000131'
        ...
        logger.error(f"Erro ao obter contagem novas cotas GMAC md-cota, error:{error}")
        raise error
...
def select_updated_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem cotas atualizadas GMAC md-cota, error:{error}")
        raise error
...
def select_adm_origin_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem cotas ganhadoras leilão GMAC md-cota, error:{error}")
        raise error
...
def select_open_sea_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem novas cotas não leilão GMAC md-cota, error:{error}")
        raise error
...
def select_group_end_date_inconsistency(md_quota_cursor: psycopg2.cursor) -> dict:
    try:
        ...
        logger.error(f"Erro ao obter contagem de inconsistências para data de "
                     f"encerramento de grupos GMAC md-cota, error:{error}")
        raise error
...
def gmac_kpi_calculates():
    ...
    try:
        ...
        logger.info(f"Quantidade de incosistências encontradas para data de encerramento de grupos GMAC: "
        ...
        logger.info(f"Total de novas ofertas para GMAC: {new_offers['new_offers']}")
