...
def insert_new_bid_stage_raw(new_bid, md_quota_cursor):
    try:
        logger.info("Inserindo dados na tabela tb_lances_volks_pre")
        query_insert_new_bid = f"""
            INSERT
            INTO
            stage_raw.tb_lances_volks_pre
            (
            pkni_grupo,
            atnd_lancebruto,
            atnd_percamortz,
        ...
        md_quota_cursor.execute(
            ...
            (
                new_bid["pkni_grupo"],
                new_bid["atnd_lancebruto"],
                new_bid["atnd_percamortz"],
                new_bid["atdt_captacao"],
                new_bid["data_info"],
                False,
                "now()",
            ),
        ...
        logger.error(
            f"Falha ao tentar inserir dados na tabela tb_lances_volks_pre: {error}"
        )
        raise error
...
def read_excel_data(s3_path, file_name, md_quota_cursor):
    ...
    for sheet in df_sheets.sheet_names:
        ...
        if sheet == "Cotas Contempladas Nas 3 Ultima":
            event_detail_type = "from_sftp_to_stage_raw_volks-groups"
            for index, row in df.iterrows():
                new_bid = {
                    "pkni_grupo": row["pkni_grupo"],
                    "atnd_lancebruto": row["atnd_lancebruto"]
                    .replace("R$", "")
                    .replace(".", "")
                    .replace(",", "."),
                    "atnd_percamortz": row["atnd_percamortz"],
                    "atdt_captacao": try_parsing_date(row["atdt_captacao"]),
                    "data_info": row["data_info"],
                }
                insert_new_bid_stage_raw(new_bid, md_quota_cursor)

            return event_detail_type


def from_sftp_to_stage_raw_bids():
