...
def switch_status(key: str, value: int) -> int:
    status = {
        "NORMAL": 1,
        "NÃO CONTEMPLADO 1 P ATRASO": 4,
        'NÃO CONTEMPLADO 2 P ATRASO': 1,
        'CONTEMPLADO S-BEM 1 P ATRASO': 1,
        'CONTEMPLADO S-BEM 2 P ATRASO': 1,
        "CONTEMPLADO S-BEM 3 P ATRASO": 1,
        "QUITADO": 6,
    }
...
def select_quotas_pos_stage_raw(cursor: psycopg2.cursor) -> None:
    try:
        logger.info("Buscando quotas na tb_quotas_santander_pos")
        query = (
            """
            SELECT *
            FROM stage_raw.tb_quotas_santander_pos
            WHERE is_processed is FALSE;
            """
        )
        cursor.execute(query)
    except Exception as error:
        logger.error(f"Error ao busca quota na tb_quotas_santander_pos, error:{error}")
        raise error
...
def update_quota_pl_quota(cursor: psycopg2.cursor, row_dict: dict, quota_id: int,
    ...
    try:
        ...
        query = (
            ...
            SET quota_number = {row_dict['cd_cota']},
                check_digit = {row_dict['cd_versao_cota']},
                is_contemplated = {is_contemplated},
                contract_number = {row_dict['nr_contrato']},
                modified_at = now(),
...
def insert_quota_pl_quota(md_quota_connection: psycopg2.connection, cursor: psycopg2.cursor, row_dict: dict,
    ...
    try:
        ...
        params = (
            ...
            row_dict.get('dt_contemplacao'),
            external_reference,
            id_adm,
            group_id,
            origin_id,
            row_dict.get('nr_contrato'),
            row_dict.get('cs_cota'),
            row_dict.get('cd_versao_cota'),
            is_contemplated,
            status_type,
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                ...
                row_dict['cd_cota'],
                row_dict['pc_tx_adm'],
                row_dict['pc_pago_contrato'],
                row_dict['vl_saldo_devedor'],
                ASSET_TYPE_ID,
                "now()",
                'now()',
                'now()',
...
def insert_group(cursor: psycopg2.cursor, id_adm: int, group: str) -> dict:
    try:
        logger.info(f"Inserindo novo grupo no md-cota: {group}")
        query = (
            """
        ...
        group = get_table_dict(cursor, group_inserted)
...
def select_quota_history_detail(cursor: psycopg2.cursor, quota_id: int) -> dict:
    try:
        ...
        result = cursor.fetchall()
        quota_dict = get_table_dict(cursor, result)
        logger.info("Histórico da cota encontrado com sucesso!")
...
def update_stage_raw(cursor: psycopg2.cursor, id_quotas: int) -> None:
    try:
        ...
        query_update_stage_raw = f"""
            UPDATE stage_raw.tb_quotas_santander_pos 
            SET is_processed = true
            WHERE id_quotas_santander_pos = {id_quotas};
        """
...
def select_pl_quota_owner(cursor: psycopg2.cursor, quota_id: int) -> dict:
    try:
        ...
        logger.error(f"Error ao busca dados pl_quota_owner: {error}")
        raise error
...
def update_pl_quota_owner(cursor: psycopg2.cursor, quota_id: int) -> None:
    try:
        ...
        logger.error(f"Erro aos fazer update na pl_quota_owner: {error}")
        raise error
...
def insert_pl_quota_owner(cursor: psycopg2.cursor, quota_id: int, person_code: str) -> None:
    try:
        ...
        logger.error(f"Erro ao inserir dados na pl_quota_owner: {error}")
        raise error


def update_owner_md_quota(cursor: psycopg2.cursor, quota_id: int) -> None:
    data_owner = select_pl_quota_owner(cursor, quota_id)
    person_code = '0000000000'
    if data_owner:
...
def insert_field_history_detail(history_detail: dict, row_dict: dict) -> dict:
    history_detail['old_quota_number'] = row_dict['cd_cota']
    history_detail['asset_value'] = row_dict['vl_bem_atual']
    history_detail['per_adm_paid'] = row_dict['pc_tx_adm']
    history_detail['per_amount_paid'] = row_dict['pc_pago_contrato']
    history_detail['amnt_to_pay'] = row_dict['vl_saldo_devedor']
...
def get_group_id(cursor: psycopg2.cursor, id_adm: int, group_exist: dict,
                 groups_list: list, group: str) -> int:
    try:
        group_id = group_exist['group_id']
    except TypeError:
        group_inserted = insert_group(cursor, id_adm, group)
...
def process_row(row_dict: dict, cursor: psycopg2.cursor, md_quota_connection: psycopg2.connection,
    ...
    is_contemplated = True if row_dict['dt_contemplacao'] is not None else False
    group = cd_group_right_justified(row_dict['cd_grupo'])
    status_type = switch_status(row_dict['nm_situacao_cobranca'], 5)
    group_exist = get_dict_by_id(group, groups_list, "group_code")
    quota_exist = get_dict_by_id(row_dict['nr_contrato'], quotas_list, "contract_number")
    external_reference = row_dict['nr_contrato']

    group_id = get_group_id(cursor, id_adm, group_exist, groups_list, group)
    if quota_exist:
    ...
    update_stage_raw(cursor, row_dict['id_quotas_santander_pos'])
    update_owner_md_quota(cursor, quota_id)
...
def quota_ingestion_santander_pos() -> None:
    ...
    try:
        ...
        logger.error(f"Erro ao processar dados da tb_quotas_santander_pos. Error{error}")
        raise error
    finally:
        connection.close()
        cursor.close()
