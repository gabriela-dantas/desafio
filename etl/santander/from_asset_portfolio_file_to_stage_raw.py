...
#    "file_key": "santander/asset-portfolio/received/partner-report-2023-11-13.csv",
#    "event_bus_name": "md"
# }
...
def get_file_data() -> Tuple[pd.DataFrame, str, str]:
    ...
    try:
        ...
        df = pd.read_csv(StringIO(data), sep=';', encoding='ISO-8859-1', decimal=",")
...
def process_columns(df: pd.DataFrame) -> List[dict]:
    ...
    for column in df.columns:
        new_column = unidecode(str(column)).lower().replace(" ", '')
        renamed_columns[column] = new_column

    df.rename(columns=renamed_columns, inplace=True)
    df = df.drop("", axis=1)

    for column in ["dt_pgto_pri_parcela", "dt_entrega_bem", "dt_contemplacao"]:
...
def insert(rows: List[dict]) -> None:
    ...
    with md_quota_connection.cursor() as cursor:
        try:
            sql_query = f'INSERT INTO stage_raw.tb_quotas_santander_pos ({colum_names}) VALUES ({values})'
            logger.debug(f"Executando query:\n{sql_query}")
            cursor.executemany(sql_query, rows)
            md_quota_connection.commit()
            logger.info("Dados inseridos com sucesso.")
        except Exception as error:
            logger.error(f"Erro na inserção na tabela de tb_quotas_santander_pos no stage_raw, error:{error}")
            raise error
        finally:
            cursor.close()
...
def put_event():
    etl_name = "quota_ingestion_santander_pos"
