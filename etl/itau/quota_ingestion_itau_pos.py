...
def select_quotas_pos_stage_raw(cursor: psycopg2.cursor) -> None:
    try:
        logger.info("Buscando quotas na tb_quotas_itau_pos")
        query = (
            """
            SELECT *
            FROM stage_raw.tb_quotas_itau_pos
            WHERE is_processed is FALSE;
            """
        )
        cursor.execute(query)
    except Exception as error:
        logger.error(f"Error ao busca quota na tb_quotas_itau_pos, error:{error}")
        raise error
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                quota_id,
                row_dict['cota'],
                row_dict['valor_bem_atual'],
                row_dict['perc_fc_pago'],
                row_dict['perc_saldo_devedor'],
                row_dict['val_saldo_devedor'],
                row_dict['data_info'],
                ASSET_TYPE_ID,
                "now()",
                'now()',
...
def update_stage_raw(cursor: psycopg2.cursor, id_quotas: int) -> None:
    try:
        ...
        query_update_stage_raw = f"""
            UPDATE stage_raw.tb_quotas_itau_pos 
            SET is_processed = true
            WHERE id_quotas_itau_pos = {id_quotas};
        """
...
def insert_field_history_detail(history_detail: dict, row_dict: dict) -> dict:
    ...
    history_detail['amnt_to_pay'] = row_dict['val_saldo_devedor']
    history_detail['per_total_amount_to_pay'] = row_dict['perc_saldo_devedor']
    history_detail['per_mutual_fund_paid'] = row_dict['perc_fc_pago']
    history_detail['asset_value'] = row_dict['valor_bem_atual']
...
def update_owner_md_quota(cursor: psycopg2.cursor, row_dict: dict, quota_id: int) -> None:
    ...
    dt_fund = data_fund()
    key = row_dict["cpf_cnpj_consorciado"].replace('.', '').replace('-', '').replace('/', '')
...
def process_row(row_dict: dict, cursor: psycopg2.cursor, md_quota_connection: psycopg2.connection,
    ...
    group_deadline = row_dict['prazo_grupo']
    is_contemplated = True if row_dict['contemplacao'] == 'S' else False
    status_type = switch_status(row_dict['situacao_de_cobranca'], 5)
    ...
    update_stage_raw(cursor, row_dict['id_quotas_itau_pos'])
...
def quota_ingestion_itau_pos() -> None:
    ...
    try:
        ...
        logger.error(f"Erro ao processar dados da tb_quotas_itau_pos. Error{error}")
        raise error
    finally:
        connection.close()
        cursor.close()
