...
def select_data_source_id(cursor: psycopg2.cursor):
    try:
        ...
        cursor.execute(query_select_data_source)
        query_result_data_source = cursor.fetchall()
        data_source_dict = get_table_dict(
            cursor, query_result_data_source
        )
...
def select_quotas_pos_stage_raw(cursor: psycopg2.cursor) -> None:
    try:
        logger.info("Buscando quotas na tb_quotas_porto_pos")
        query = (
            """
            SELECT *
            FROM stage_raw.tb_quotas_porto_pos
            WHERE is_processed is FALSE;
            """
        )
        cursor.execute(query)
    except Exception as error:
        logger.error(f"Error ao busca quota na tb_quotas_porto_pos, error:{error}")
        raise error
...
def insert_quota_pl_quota(md_quota_connection: psycopg2.connection, cursor: psycopg2.cursor, row_dict: dict,
    ...
    try:
        ...
        params = (
            ...
            group_id,
            origin_id,
            row_dict.get('nro_contrato_adesao'),
            row_dict.get('cota'),
            is_contemplated,
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                quota_id,
                row_dict['cota'],
                row_dict['valor_credito_atual'],
                row_dict['saldo_devedor_perc_cota'],
                row_dict['saldo_devedor_cota'],
                row_dict['data_info'],
                ASSET_TYPE_ID,
                "now()",
                'now()',
...
def insert_group(cursor: psycopg2.cursor, id_adm: int, group_code: str) -> dict:
    try:
        logger.info(f"Inserindo novo grupo no md-cota: {group_code}")
        query = (
            """
...
def update_stage_raw(cursor: psycopg2.cursor, id_quotas: int) -> None:
    try:
        ...
        query_update_stage_raw = f"""
            UPDATE stage_raw.tb_quotas_porto_pos 
            SET is_processed = true
            WHERE id_quotas_porto_pos = {id_quotas};
        """
...
def insert_field_history_detail(history_detail: dict, row_dict: dict) -> dict:
    ...
    history_detail['amnt_to_pay'] = row_dict['saldo_devedor_cota']
    history_detail['per_total_amount_to_pay'] = row_dict['saldo_devedor_perc_cota']
    history_detail['asset_value'] = row_dict['valor_credito_atual']
...
def data_fund() -> dict:
    data = {'SPS CORP I -FUNDO INVEST.DIR.CREDITORIOS': '0000061932',
            'BAZAR DO CONSORCIO TECNOLOGIA LTDA': '0000061935',
            'ATIVOS ESPECIAIS II - F.INVEST.DIR.CRED': '0000061936',
            'ATIVOS ESPECIAIS III - FUND INV DIR CRED': '0000227564'}
    return data
...
def update_owner_md_quota(cursor: psycopg2.cursor, row_dict: dict, quota_id: int) -> None:
    ...
    dt_fund = data_fund()
    person_code = dt_fund[row_dict["nome_razao"]]
    if data_owner:
...
def get_group_id(cursor: psycopg2.cursor, id_adm: int, group_exist: dict, groups_list: list, group: str) -> int:
    try:
...
def process_row(row_dict: dict, cursor: psycopg2.cursor, md_quota_connection: psycopg2.connection,
                groups_list: list, quotas_list: list, id_adm: int,
                data_source_id: int) -> None:
    is_contemplated = True if row_dict['status_cliente'] in ('CONTEMPLADO C/ BEM', 'CONTEMPLADO S/ BEM') else False
    status_type = switch_status(row_dict['status'], 5)
    group = cd_group_right_justified(row_dict['grupo'])
    group_exist = get_dict_by_id(group, groups_list, "group_code")
    quota_exist = get_dict_by_id(row_dict['nro_contrato_adesao'], quotas_list, "contract_number")
    external_reference = row_dict['uuid']
    ...
    update_stage_raw(cursor, row_dict['id_quotas_porto_pos'])
...
def quota_ingestion_porto_pos() -> None:
    ...
    try:
        ...
        logger.error(f"Erro ao processar dados da tb_quotas_porto_pos. Error{error}")
        raise error
    finally:
        connection.close()
        cursor.close()
