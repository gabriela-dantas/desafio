    ...
    try:
        query_select_new_quotas_md_quota = (
            ...
            FROM
                ...
                WHERE
                    pa.administrator_code = '0000000234'
        ...
        logger.error(f"Erro ao obter contagem novas cotas Santander md-cota, error:{error}")
        raise error
...
def select_updated_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem cotas atualizadas Santander md-cota, error:{error}")
        raise error
...
def select_new_auction_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem cotas ganhadoras leilão Santander md-cota, error:{error}")
        raise error
...
def select_new_no_auction_quotas_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        ...
        logger.error(f"Erro ao obter contagem novas cotas não leilão Santander md-cota, error:{error}")
        raise error
...
def select_group_end_date_inconsistency(md_quota_cursor: psycopg2.cursor) -> dict:
    try:
        ...
        logger.error(f"Erro ao obter contagem de inconsistências para data de "
                     f"encerramento de grupos Santander md-cota, error:{error}")
        raise error
...
def select_auction_quotas_winner_md_quota(md_quota_cursor: psycopg2.cursor) -> int:
    try:
        query_select_auction_quotas_quotas_winner_md_quota = (
            ...
            FROM
                ...
                FROM
                    md_cota.pl_quota pq 
                LEFT JOIN md_cota.pl_administrator pa on pa.administrator_id = pq.administrator_id 
                LEFT join md_cota.pl_quota_owner pqo on pqo.quota_id = pq.quota_id
                WHERE
                    pa.administrator_code = '0000000234'
                    AND pq.CREATED_AT >= (CURRENT_DATE - interval '24 hour')::timestamp
                    AND pq.quota_origin_id = 1
                    AND pqo.valid_to is null
                    AND pqo.person_code is not null) as quotas
            group BY TO_CHAR(quotas.MIN_CREATED_AT - interval '3 hour', 'YYYY-MM-DD')
            order by 1 desc limit 7;
        """)
        ...
        logger.error(f"Erro ao obter contagem de cotas ganhas no leilão Santander md-cota, error:{error}")
        raise error
...
def select_bpm_new_cards(bpm_cursor: psycopg2.cursor) -> dict:
    try:
...
def insert_data_into_dynamo(new_quotas_md_quota: int, updated_quotas: int, new_auction_quotas: int,
                            ...
                            new_cards: dict, new_offers: dict, auction_winner_quotas: int) -> None:
...
def santander_kpi_calculates():
    ...
    try:
        ...
        logger.info(f"Quantidade de incosistências encontradas para data de encerramento de grupos Santander: "
        ...
        logger.info(f"Total de novas ofertas para Santander: {new_offers['new_offers']}")
