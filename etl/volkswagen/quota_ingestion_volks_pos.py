...
def select_quotas_pos_stage_raw(cursor: psycopg2.cursor) -> None:
    try:
        logger.info("Buscando quotas na tb_quotas_volks_pos")
        query = (
            """
            SELECT *
            FROM stage_raw.tb_quotas_volks_pos
            WHERE is_processed is FALSE;
            """
        )
        cursor.execute(query)
    except Exception as error:
        logger.error(f"Error ao busca quota na tb_quotas_volks_pos, error:{error}")
        raise error


def update_quota_pl_quota(cursor: psycopg2.cursor, row_dict: dict, quota_id: int,
                          is_contemplated: bool, status_type: int) -> None:
    if row_dict['dt_contemplacao']:
        dt_contemplacao = f"'{row_dict['dt_contemplacao']}'"
    else:
    ...
    try:
        ...
        query = (
            ...
            SET quota_status_type_id = {status_type},
                contemplation_date = {dt_contemplacao},
                quota_number = {row_dict['cota']},
                version_id = {row_dict['vs']},
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                ...
                row_dict['cota'],
                row_dict['vl_bem'],
                row_dict['pe_fc_pagar'],
                row_dict['vl_sd_devedor'],
                row_dict['data_info'],
                ASSET_TYPE_ID,
                "now()",
                'now()',
...
def update_stage_raw(cursor: psycopg2.cursor, id_quotas: int) -> None:
    try:
        ...
        query_update_stage_raw = f"""
            UPDATE stage_raw.tb_quotas_volks_pos 
            SET is_processed = true
            WHERE id_quotas_volks_pos = {id_quotas};
        """
...
def insert_field_history_detail(history_detail: dict, row_dict: dict) -> dict:
    ...
    history_detail['per_total_amount_to_pay'] = row_dict['pe_pago']
    history_detail['overdue_percentage'] = row_dict['pe_atraso']
    history_detail['per_mutual_fund_to_pay'] = row_dict['pe_fc_pagar']
    history_detail['installments_paid_number'] = row_dict['qtd_pcl_paga']
    history_detail['overdue_installments_number'] = row_dict['qtd_pcl_atraso']
    history_detail['asset_value'] = row_dict['vl_bem']
    history_detail['amnt_to_pay'] = row_dict['vl_sd_devedor']
...
def update_owner_md_quota(cursor: psycopg2.cursor, row_dict: dict, quota_id: int) -> None:
    ...
    dt_fund = data_fund()
    key = row_dict["inscricao_nacional"].replace('.', '').replace('-', '').replace('/', '')
...
def quota_exist_md_quota(cursor: psycopg2.cursor, quota_exist: dict, row_dict: dict,
    ...
    update_quota_pl_quota(cursor, row_dict, quota_exist['quota_id'], is_contemplated, status_type)
...
def process_row(row_dict: dict, cursor: psycopg2.cursor, md_quota_connection: psycopg2.connection,
    ...
    status_type = switch_status(row_dict['situacao_cobranca'], 5)
    ...
    update_stage_raw(cursor, row_dict['id_quotas_volks_pos'])
...
def quota_ingestion_volks_pos() -> None:
